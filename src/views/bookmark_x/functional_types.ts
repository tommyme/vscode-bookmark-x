...
import {
  ...
  SerializableGroup,
  SerializableGroupBookmark,
} from "./serializable_type";
import {
  ICON_ACTIVE_GROUP,
  ICON_GROUP,
  ITEM_TYPE_BM,
...
export class BaseFunctional {
  ...
  type: string;
  children: NodeType[];
  constructor(name: string, uri: string, type: string, children: NodeType[]) {
    this.name = name;
    this.uri = uri;
    this.type = type;
...
export class Group extends BaseFunctional {
  ...
  public static fromSerialized(group: SerializableGroup): Group {
    return new Group(group.name, group.color, group.uri);
  }
  ...
  public sortGroupBookmark() {
    ...
    if (sortOption === "group first") {
      ...
      window.showInformationMessage(
        "sort fail, sort option invalid: " + sortOption,
      );
    }
  }
...
export class Bookmark extends BaseFunctional {
  ...
  public static sortByLocation(a: Bookmark, b: Bookmark): number {
    return a.fsPath.localeCompare(b.fsPath) || a.line - b.line || a.col - b.col;
  }
...
export class GroupBookmark extends Group {
  ...
  constructor(bm: Bookmark, group: Group | undefined = undefined) {
    if (!group) {
...
export class NodeUriMap extends UriMap<BaseFunctional> {
  ...
  public bookmark_num() {
    let result = 0;
    for (let key of this.keys()) {
      if (this.get(key).type === ITEM_TYPE_BM) {
        result++;
      }
    }
    return result;
...
export class RootGroup extends Group {
  ...
  public get_bm_with_under_fspath(fspath: string): Array<Bookmark> {
    ...
    this.cache.keys().forEach((key) => {
      ...
      if (
        node instanceof Bookmark &&
        util.isPathAEqUnderPathB(node.fsPath, fspath)
      ) {
        res.push(node);
      }
    });
